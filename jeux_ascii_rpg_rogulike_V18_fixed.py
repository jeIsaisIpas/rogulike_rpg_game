...
- ameliorer le menu admin pour avoir plus d'aventage en run
-7 Nouvelles zones (derniere zone loboratoire du vide (zone n°10)) (troix enemi et deux boss par zone)
    pour Augmente la durais du jeux (10 etage par zone donc etage max 30 => 100)
-retirer le text pour les lvl et exp car ne sert a rien depuit la V12
-remplacer les plan de la V18 par les plan pour la V19
-si IA utiliser aucun emodji accepter
-maitre des couleur unique pour la barre de vie du player et de l'enemi et du mana
-ajouter des type de salle(
    salle de feu de camp (repos gratuit, tu l’as déjà en événement, tu peux en faire un type de chemin)
    salle de élite : ennemi plus fort mais plus de récompenses
    salle de trésor garantie )
-un recap de run quand celle si se fini
-faire en sorte que le system de combat soi plus opti car la l'option defance ne serre a rien 
    (réelement theriqument elle sert mais mecaniquement non)
-mettre les couleur dans le styll rgb (255, 255, 255)
...
def rgb(r, g, b):
    """Retourne un code de couleur ANSI 24 bits (RGB)."""
    return f"[38;2;{r};{g};{b}m"

def rgb_bg(r, g, b):
    """Retourne un code de fond ANSI 24 bits (RGB)."""
    return f"[48;2;{r};{g};{b}m"

# Redéfinition des couleurs en style RGB pour la V18
...
# ======================================================
# Données de base : classes joueurs, armes, reliques, zones
# ======================================================
PLAYER_CLASSES: Dict[str, Dict] = {
    "1": {"nom": "Guerrier", "pv": 35, "attaque": 6, "defense": 3, "mana": 5, "pouvoir": "Techniques de combat"},
    "2": {"nom": "Mage", "pv": 22, "attaque": 4, "defense": 1, "mana": 15, "pouvoir": "Arcanes élémentaires"},
    "3": {"nom": "Voleur", "pv": 28, "attaque": 5, "defense": 2, "mana": 8, "pouvoir": "Arts furtifs"},
    "4": {"nom": "Paladin", "pv": 40, "attaque": 5, "defense": 4, "mana": 8, "pouvoir": "Lumière sacrée"},
    "5": {"nom": "Rôdeur", "pv": 30, "attaque": 6, "defense": 2, "mana": 7, "pouvoir": "Maîtrise de l'arc"},
    "6": {"nom": "Nécromancien", "pv": 24, "attaque": 5, "defense": 1, "mana": 18, "pouvoir": "Magies interdites"},
    "7": {"nom": "Assassin de l'Ombre", "pv": 26, "attaque": 7, "defense": 2, "mana": 10, "pouvoir": "Arts des ombres"},
    "8": {"nom": "Berserker", "pv": 38, "attaque": 8, "defense": 2, "mana": 4, "pouvoir": "Rage sanguinaire"},
    "9": {"nom": "Barde", "pv": 27, "attaque": 4, "defense": 2, "mana": 14, "pouvoir": "Chants inspirants"},
    "10": {"nom": "Moine", "pv": 32, "attaque": 5, "defense": 3, "mana": 9, "pouvoir": "Arts martiaux spirituels"},
    "11": {"nom": "Invocateur", "pv": 23, "attaque": 3, "defense": 1, "mana": 20, "pouvoir": "Invocations ancestrales"},
}
...
WEAPONS: List[Weapon] = [
    ...
    Weapon("Arc des ombres", atk_bonus=1, crit_bonus=0.15,
           description="Un arc léger : peu d'attaque brute mais énormes chances critiques."),
]
...
# ======================================================
# Cartes (système de progression V17)
# ======================================================
@dataclass
class Card:
    nom: str
    description: str
    rarete: str  # "Commune", "Rare", "Épique", "Légendaire"
...
CARDS_POOL: List[Card] = [
    # Communes génériques
    Card("Endurance", "+5 PV max", "Commune", bonus_pv_max=5),
    Card("Muscles tendus", "+1 ATQ", "Commune", bonus_attaque=1),
    Card("Posture défensive", "+1 DEF", "Commune", bonus_defense=1),
    Card("Méditation", "+2 Mana max", "Commune", bonus_mana_max=2),
    Card("Flasque de secours", "+1 potion", "Commune", bonus_potions=1),
    ...
    # Épiques génériques
    Card("Robustesse surnaturelle", "+15 PV max", "Épique", bonus_pv_max=15),
    Card("Force brute", "+3 ATQ", "Épique", bonus_attaque=3),
    Card("Mur de fer", "+3 DEF", "Épique", bonus_defense=3),
    Card("Réservoir mystique", "+6 Mana max", "Épique", bonus_mana_max=6),
    Card("Spécialiste des fioles", "+3 potions", "Épique", bonus_potions=3),

    # Légendaires génériques
    Card("Colosse immortel", "+25 PV max, +2 DEF", "Légendaire", bonus_pv_max=25, bonus_defense=2),
    Card("Lame du destin", "+5 ATQ, +10% crit", "Légendaire", bonus_attaque=5, bonus_critique=0.10),
    Card("Maître arcanique", "+8 Mana max, +2 ATQ", "Légendaire", bonus_mana_max=8, bonus_attaque=2),

    # Cartes spécifiques de classe (exemples)
    Card("Discipline guerrière", "+3 ATQ (Guerrier)", "Rare",
         classes_autorisees=["Guerrier"], bonus_attaque=3),
    Card("Études occultes", "+5 Mana max (Nécromancien)", "Rare",
         classes_autorisees=["Nécromancien"], bonus_mana_max=5),
    Card("Instinct furtif", "+10% crit (Voleur / Assassin de l'Ombre)", "Rare",
         classes_autorisees=["Voleur", "Assassin de l'Ombre"], bonus_critique=0.10),
]


# Ennemis & zones
CATACOMBES_ENNEMIS = [
    {"nom": "Rat géant", "pv": 12, "attaque": 3, "defense": 1, "gold": (3, 7), "xp": (5, 8)},
    {"nom": "Gobelin", "pv": 18, "attaque": 4, "defense": 1, "gold": (6, 12), "xp": (8, 12)},
    {"nom": "Squelette", "pv": 20, "attaque": 5, "defense": 2, "gold": (10, 18), "xp": (10, 14)},
]

FORET_ENNEMIS = [
    {"nom": "Loup sombre", "pv": 22, "attaque": 6, "defense": 2, "gold": (10, 18), "xp": (12, 18)},
    {"nom": "Dryade corrompue", "pv": 26, "attaque": 5, "defense": 3, "gold": (12, 20), "xp": (14, 20)},
    {"nom": "Ent déchaîné", "pv": 30, "attaque": 7, "defense": 3, "gold": (15, 24), "xp": (16, 22)},
]

CITADELLE_ENNEMIS = [
    {"nom": "Garde runique", "pv": 32, "attaque": 8, "defense": 4, "gold": (18, 26), "xp": (18, 26)},
    {"nom": "Mage de cristal", "pv": 28, "attaque": 9, "defense": 3, "gold": (20, 30), "xp": (20, 28)},
    {"nom": "Chevalier spectral", "pv": 36, "attaque": 9, "defense": 4, "gold": (24, 34), "xp": (22, 30)},
]

CATACOMBES_BOSSES = [
    {"nom": "Seigneur des Os", "pv": 45, "attaque": 7, "defense": 3, "gold": (30, 45), "xp": (35, 50)},
    {"nom": "Liche des Profondeurs", "pv": 40, "attaque": 9, "defense": 2, "gold": (30, 45), "xp": (35, 50)},
]

FORET_BOSSES = [
    {"nom": "Esprit de la Forêt Putride", "pv": 55, "attaque": 9, "defense": 4, "gold": (40, 60), "xp": (45, 65)},
    {"nom": "Reine Araignée", "pv": 50, "attaque": 10, "defense": 3, "gold": (40, 60), "xp": (45, 65)},
]

CITADELLE_BOSSES = [
    {"nom": "Seigneur Runique", "pv": 70, "attaque": 11, "defense": 5, "gold": (60, 80), "xp": (70, 90)},
    {"nom": "Champion du Vide", "pv": 65, "attaque": 12, "defense": 4, "gold": (60, 80), "xp": (70, 90)},
]

ZONES = [
    {"nom": "Catacombes", "debut": 1, "fin": 10, "ennemis": CATACOMBES_ENNEMIS, "bosses": CATACOMBES_BOSSES},
    {"nom": "Forêt Maudite", "debut": 11, "fin": 20, "ennemis": FORET_ENNEMIS, "bosses": FORET_BOSSES},
    {"nom": "Citadelle Runique", "debut": 21, "fin": 30, "ennemis": CITADELLE_ENNEMIS, "bosses": CITADELLE_BOSSES},
]
...
# ======================================================
# Arbre de talents (skill tree) V17.2
# ======================================================
SKILL_TREE: Dict[str, Dict] = {
    ...
    "atk1": {
        "nom": "+1 ATQ de base",
        "description": "Augmente l'attaque de base de 1.",
        "cout": 5,
        "type": "stat",
        "cible": "attaque_bonus",
        "valeur": 1,
    ...
    "shop1": {
        "nom": "+1 niveau de chance de boutique",
    ...
    "mystere1": {
        "nom": "+1 niveau de chance d'événement mystère",
    ...
    "class_rodeur": {
        "nom": "Débloquer Rôdeur",
        "description": "Permet de choisir la classe Rôdeur au début de la run.",
        "cout": 8,
        "type": "unlock_class",
        "cible": "Rôdeur",
        "valeur": 1,
    ...
    "class_necromancien": {
        "nom": "Débloquer Nécromancien",
        "description": "Permet de choisir la classe Nécromancien au début de la run.",
        "cout": 9,
        "type": "unlock_class",
        "cible": "Nécromancien",
        "valeur": 1,
    ...
    "class_assassin_ombre": {
        "nom": "Débloquer Assassin de l'Ombre",
    ...
    "class_barde": {
        "nom": "Débloquer Barde",
        "description": "Permet de choisir la classe Barde au début de la run.",
        "cout": 8,
        "type": "unlock_class",
        "cible": "Barde",
        "valeur": 1,
    ...
    "class_moine": {
        "nom": "Débloquer Moine",
        "description": "Permet de choisir la classe Moine au début de la run.",
        "cout": 9,
        "type": "unlock_class",
        "cible": "Moine",
        "valeur": 1,
    ...
    "class_invocateur": {
        "nom": "Débloquer Invocateur",
        "description": "Permet de choisir la classe Invocateur au début de la run.",
        "cout": 10,
        "type": "unlock_class",
        "cible": "Invocateur",
        "valeur": 1,
...
class Meta:
    ...
    classes_debloquees: List[str] = field(default_factory=lambda: ["Guerrier", "Mage", "Voleur"])
    ...
    def charger(cls, filename: str) -> "Meta":
        ...
        meta = cls(
            ...
            classes_debloquees=data.get("classes_debloquees", ["Guerrier", "Mage", "Voleur"]),
...
class Game:
    ...
    def creer_joueur(self) -> Player:
        ...
        print(COLOR_MAGENTA + "=" * 30 + COLOR_RESET)
        print()
        nom = input("Entre le nom de ton héros : ") or "Héros"
        print()
        ...
        for _, infos in PLAYER_CLASSES.items():
            if infos["nom"] in self.meta.classes_debloquees:
        ...
        if not classes_disponibles:
            ...
            for _, infos in PLAYER_CLASSES.items():
                if infos["nom"] in ("Guerrier", "Mage", "Voleur"):
        ...
        for key, infos in classes_disponibles.items():
            print(
                f" {key}) {infos['nom']} "
                f"PV:{infos['pv']} ATQ:{infos['attaque']} "
        ...
        pv_base = base['pv'] + self.meta.pv_bonus
        mana_base = base['mana'] + self.meta.mana_bonus
        attaque_base = base['attaque'] + self.meta.attaque_bonus
    ...
    def afficher_etat_joueur(self) -> None:
        ...
        if j.classe == "Nécromancien" and j.invocation_squelette_tours > 0:
    ...
    def appliquer_degats_joueur(self, degats: int) -> int:
        ...
        if random.random() < dodge_chance:
            print("Tu esquives habilement l'attaque !")
            return 0
        j.pv -= degats
        return degats
    ...
    def tour_joueur(self, ennemi: Enemy) -> str:
        ...
        while True:
            ...
            if choix == "1":
                ...
                if j.classe == "Voleur" and random.random() < 0.25:
    ...
    def choisir_pouvoir_et_utiliser(self, j: Player, ennemi: Enemy):
        # Nécromancien : 2 sorts
        if j.classe == "Nécromancien":
            while True:
                clear_console()
                print("=== Sorts de Nécromancien ===")
        ...
        # Guerrier : 2 techniques
        if j.classe == "Guerrier":
            while True:
                clear_console()
                print("=== Techniques du Guerrier ===")
                print(f"Mana actuel : {j.mana}/{j.mana_max}")
        ...
        # Voleur : 2 arts furtifs
        if j.classe == "Voleur":
            while True:
                clear_console()
                print("=== Arts furtifs du Voleur ===")
                print(f"Mana actuel : {j.mana}/{j.mana_max}")
        ...
        # Moine : 2 techniques spirituelles
        if j.classe == "Moine":
            while True:
                clear_console()
                print("=== Arts martiaux spirituels (Moine) ===")
                print(f"Mana actuel : {j.mana}/{j.mana_max}")
        ...
        # Invocateur : 2 incantations
        if j.classe == "Invocateur":
            while True:
                clear_console()
                print("=== Incantations de l'Invocateur ===")
                print(f"Mana actuel : {j.mana}/{j.mana_max}")
    ...
    def utiliser_pouvoir_special(self, j: Player, ennemi: Enemy, ability_id: int = 1):
        ...
        if j.classe == "Voleur":
            if ability_id == 1:
                ...
                if random.random() < 0.4:
                    ennemi.statuts.poison += 2
                    print("Ton attaque furtive empoisonne l'ennemi !")
    ...
    def ennemi_competence_speciale(self, ennemi: Enemy) -> None:
        ...
        if "Seigneur" in ennemi.nom or "Champion" in ennemi.nom or "Reine" in ennemi.nom:
            ...
            return
        # Ennemis normaux : buff ou débuff
    ...
    def tour_ennemi(self, ennemi: Enemy, action_joueur: str) -> None:
        ...
        if random.random() < 0.15:
            j.statuts.poison += 2
            print("L'attaque de l'ennemi t'empoisonne !")
        elif random.random() < 0.15:
            j.statuts.brulure += 2
            print("L'attaque de l'ennemi te brûle !")
    ...
    def tirer_cartes(self, n: int = 3) -> List[Card]:
        ...
        for _ in range(n):
            r = random.random()
            if r < 0.60:
                rarete_visee = "Commune"
            elif r < 0.85:
                rarete_visee = "Rare"
            elif r < 0.97:
                rarete_visee = "Épique"
            else:
                rarete_visee = "Légendaire"
    ...
    def combat(self, ennemi: Enemy) -> bool:
        ...
        while j.est_vivant() and ennemi.est_vivant():
            ...
            if not ennemi.est_vivant():
                ...
                attendre_entree()
                return True
            if j.classe == "Nécromancien" and j.invocation_squelette_tours > 0 and ennemi.est_vivant():
    ...
    def boutique(self) -> None:
        ...
        while True:
            ...
            print(" 2) Augmenter l'attaque (+1) - 25 or")
            ...
            if choix == "2":
                if j.gold < 25:
                    ...
                    j.gold -= 25
                    j.attaque += 1
                    print("Ton attaque augmente de 1 ! (pour cette run)")
                attendre_entree()
                continue
            if choix == "3":
                if j.gold < 25:
    ...
    def donner_arme_random(self) -> None:
        ...
        while True:
            ...
            print(" 3) Laisser l'arme et continuer ton chemin")
            choix = input(">> ").strip()
            if choix == "1":
    ...
    def evenement_mystere(self) -> None:
        ...
        if effet == "relique":
            ...
            return
        if effet == "bon":
            choix = random.choice(["pv", "attaque", "or"])
            if choix == "pv":
                bonus = 8
                j.pv_max += bonus
                j.pv += bonus
            ...
            return
        if effet == "mauvais":
            malus = random.choice(["pv", "attaque"])
            if malus == "pv":
    ...
    def etage_roguelike(self, etage: int) -> bool:
        ...
        print("#" * 50)
        print(COLOR_CYAN + f"ETAGE {etage} - Zone : {zone['nom']}" + COLOR_RESET)
        print("#" * 50)
        ...
        if self.est_etage_boss(etage):
            ...
            resist = {}
            weak = {}
            if "Os" in boss_data['nom']:
                resist = {"physique": 0.8, "feu": 0.5}
                weak = {"glace": 1.4}
            elif "Forêt" in boss_data['nom'] or "Ent" in boss_data['nom']:
                resist = {"glace": 0.7}
                weak = {"feu": 1.5}
            elif "Runique" in boss_data['nom'] or "Cristal" in boss_data['nom']:
                resist = {"sacre": 0.7}
                weak = {"ombre": 1.4}
            ennemi = Enemy(
                nom=boss_data['nom'],
                pv=int(boss_data['pv'] * diff),
                pv_max=int(boss_data['pv'] * diff),
                attaque=int(boss_data['attaque'] * diff),
        ...
        while True:
            ...
            print("(Astuce : 1 ou 2 pour choisir un chemin, 'i' pour gérer tes armes, 'admin' pour le menu secret.)")
            ...
            if chemin_type == "combat":
                ...
                resist = {}
                weak = {}
                if "Squelette" in ennemi_data['nom']:
                    resist = {"feu": 0.5, "physique": 0.9}
                    weak = {"glace": 1.3}
                elif "Loup" in ennemi_data['nom']:
                    resist = {"physique": 0.9}
                    weak = {"feu": 1.2}
                elif "Mage" in ennemi_data['nom']:
                    resist = {"feu": 0.8, "glace": 0.8}
                    weak = {"ombre": 1.3}
                ennemi = Enemy(
                    nom=ennemi_data['nom'],
                    pv=int(ennemi_data['pv'] * diff),
                    pv_max=int(ennemi_data['pv'] * diff),
                    attaque=int(ennemi_data['attaque'] * diff),
    ...
    def appliquer_talent(self, talent_id: str, node: Dict) -> None:
        ...
        if t_type in ("stat", "start_res", "chance") and cible:
            ...
            print(f"Le talent '{node['nom']}' renforce {cible} de {val}.")
        ...
        while True:
            ...
            if not disponibles:
                ...
                for i, (tid, node) in enumerate(disponibles, start=1):
                    print(f" {i}) {node['nom']} ({node['cout']} essence) - {node['description']}")
        ...
        while True:
            ...
            if choix == "1":
                ...
                if confirm == "oui":
                    ...
                    self.meta.sauvegarder()
                    print("Méta-progression réinitialisée.")
                else:
    ...
    def afficher_features(self) -> None:
        ...
        print("- Roguelike à étages avec 3 zones (Catacombes, Forêt Maudite, Citadelle Runique).")
        print("- Système de combat au tour par tour avec statuts (poison, brûlure, étourdissement).")
        print("- Armes et reliques avec bonus, coups critiques et vol de vie.")
        print("- Méta-progression avec essence d'âme et arbre de talents permanent.")
        print("- Classes jouables avec pouvoirs spéciaux (Guerrier, Mage, Voleur et classes avancées à débloquer).")
